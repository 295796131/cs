    ...
    respond_to do |format|
      ...
      format.xls {
        ...
        @grades.each_with_index do |grade,i|
          #puts(grade.user.num)
          sheet1.row(i+1).push  grade.user.num,
                                grade.user.name, grade.user.major, grade.user.department,
                                grade.course.name, grade.grade
        end
        book.write 'wqs.xls'
        send_file 'wqs.xls',:type => "application/vnd.ms-excel",
                  :filename => "#{@course.name}.xls", :stream => false
        return
      }
    end
  end
